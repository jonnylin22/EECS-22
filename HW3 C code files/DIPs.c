/********************************************************************************/
/* DIPs.c : program file for DIP function definitions from assignment 2	 	*/
/* 	BlackNWhite(), Negative(), ColorFilter(), Edge(), Shuffle(),		*/
/*	VFlip(), HMirror(), Pixelate()						*/
/********************************************************************************/
		...
		for (int j = 0 ; j < (HEIGHT - (HEIGHT % block_size)) ; j +=block_size ) { // 72 subsections down of 4 tall
			...
			for (int sub_i = 0 ; sub_i < block_size ; sub_i++) {
				for (int sub_j = 0 ; sub_j < block_size ; sub_j++) {
					...
					r_sum += R[i + sub_i][j + sub_j];  
					g_sum += G[i + sub_i][j + sub_j];  
					b_sum += B[i + sub_i][j + sub_j];  
				}
			} 
			...
			for (int sub_i = 0 ; sub_i < block_size ; sub_i++) {
				for (int sub_j = 0; sub_j < block_size ; sub_j++) {
					R[i+sub_i][j + sub_j] = r_avg;
					G[i+sub_i][j + sub_j] = g_avg;
					B[i+sub_i][j + sub_j] = b_avg;
				}
			}
		} 
	}
